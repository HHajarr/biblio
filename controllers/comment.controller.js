...
const createComment = async (req, res) => {
    try{
        const {content, created_at, verified} = req.body
        const response = await commentDB.createComment(content, created_at, verified)
        if(response.error){return res.status(500).json({message: response.error})}
        return res.status(201).json({message:"Le commentaire a été ajouté avec succès", id_comment:response.result.insertId})
    } 
...
const readCommentByUser = async (req,res) => {
  try{
    ...
    const commentResponse = await commentDB.readCommentByBook(id_user)
    if (commentResponse.error) {
    return res.status(500).json({ message: commentResponse.error });
    }
    const comments = commentResponse.result || [];
...
const updateComment = async (req, res) => {
    ...
    try{ 
        ...
        const idUser = await commentDB.getUserComment(id_comment);
        const {content, created_at, verified, id_user} = req.body
        if (idUser !== id_user) {return res.status(500).json({message: "Vous n'êtes pas autorisé à modifier ce commentaire"})}
        const response = await commentDB.updateComment(content, created_at, verified, id_comment)
        if(response.error){return res.status(500).json({message: response.error})}
        return res.status(201).json({message: `Le commentaire a été modifié avec succès`})
    } 
    catch(error){console.error("Controller error",error)
        return res.status(500).json({message: error.message})
    }
...
const deleteComment = async (req, res) => {
  try {
    const id_comment = req.params.id_comment;
    const response = await commentDB.deleteComment(id_comment);
    if (response.error) {
